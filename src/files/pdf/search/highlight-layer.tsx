...
const Highlight = ({ highlight, isCurrent }: HighlightProps) => {
  const { smoothScrolling } = useFileViewerConfig();

  const scrollRef = useCallback(
    (element: HTMLSpanElement | null) => {
      if (element !== null && isCurrent) {
        element.scrollIntoView({ behavior: smoothScrolling ? 'smooth' : 'instant', block: 'center' });
      }
    },
    [isCurrent, smoothScrolling],
  );
